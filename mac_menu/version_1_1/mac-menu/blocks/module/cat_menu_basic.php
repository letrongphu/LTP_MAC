...
if (!class_exists('Mac_Category_Menu')) {
    class Mac_Category_Menu {
        function render($catMenuAttr) {
            ...
            $is_img = isset($catMenuAttr['cat_menu_is_img']) ? $catMenuAttr['cat_menu_is_img'] : "";
            $is_description = isset($catMenuAttr['cat_menu_is_description']) ? $catMenuAttr['cat_menu_is_description'] : "";
            $is_price = isset($catMenuAttr['cat_menu_is_price']) ? $catMenuAttr['cat_menu_is_price'] : "";

            $menu_item_is_img = isset($catMenuAttr['cat_menu_item_is_img']) ? $catMenuAttr['cat_menu_item_is_img'] : "";
            $menu_item_is_description = isset($catMenuAttr['cat_menu_item_is_description']) ? $catMenuAttr['cat_menu_item_is_description'] : "";
            $menu_item_is_price = isset($catMenuAttr['cat_menu_item_is_price']) ? $catMenuAttr['cat_menu_item_is_price'] : "";
                ...
                <div id="<?= isset($isCat[0]->slug_category) ? $isCat[0]->slug_category : ''  ?>" class="module-category-menu<?= $class; ?>">
                    <div class="cat-menu-text">
                        <div class="cat-menu-head">
                            ...
                            <?php endif; ?>
                            <?php if (isset($isCat[0]->price) && !empty($isCat[0]->price) && $is_price != 'off' ): ?>
                                <div class="cat-menu-price"><?= $isCat[0]->price ?></div>
                            <?php endif; ?>
                        </div>
                        <?php if (isset($isCat[0]->category_description) && !empty($isCat[0]->category_description) && $is_description != 'off' ): ?>
                            <div class="cat-menu-description"><?= $isCat[0]->category_description ?></div>
                        <?php endif; ?>
                    </div>
                    <?php if (isset($isCat[0]->featured_img) && !empty($isCat[0]->featured_img) && $is_img != 'off' ): ?>
